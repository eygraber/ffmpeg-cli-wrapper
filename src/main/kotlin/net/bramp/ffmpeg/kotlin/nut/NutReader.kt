...
class NutReader(inputStream: InputStream, private val listener: NutReaderListener) {
    ...
    try {
      ...
      while(true) {
        ...
        when(packet) {
          ...
          Packet.StartCode.SyncPoint -> {
            Packet().read(dataInputStream, startcode) // Discard for the moment
            startcode = dataInputStream.readStartCode()
          }
          Packet.StartCode.Index -> {
            Packet().read(dataInputStream, startcode) // Discard for the moment
            startcode = dataInputStream.readStartCode()
          }
          Packet.StartCode.Info -> {
            Packet().read(dataInputStream, startcode) // Discard for the moment
            startcode = dataInputStream.readStartCode()
          }
        }
  ...
  companion object {
    ...
    fun isKnownStartcode(startcode: Long): Boolean = Packet.StartCode.Companion.of(startcode) != null
  }
}
