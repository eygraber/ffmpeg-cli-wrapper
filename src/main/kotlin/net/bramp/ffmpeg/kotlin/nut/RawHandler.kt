...
object RawHandler {
  ...
  fun streamToAudioFormat(header: StreamHeaderPacket): AudioFormat {
    ...
    require(header.fourcc.size == 4) {
      "unknown fourcc value: '${StreamHeaderPacket.Companion.fourccToString(header.fourcc)}'"
    }
    ...
    if(fourcc.contentEquals(alaw)) {
      ...
      require(isFourCCLittleEndian || isFourCCBigEndian) {
        "unknown fourcc value: '${StreamHeaderPacket.Companion.fourccToString(fourcc)}'"
      }
      ...
      require(interleaving == 'D'.code.toByte()) {
        "unsupported interleaving '$interleaving' in fourcc value '${StreamHeaderPacket.Companion.fourccToString(
          fourcc,
        )}'"
      }
      ...
      when(type) {
        ...
        'F'.code.toByte() -> encoding = AudioFormat.Encoding.PCM_FLOAT
          ...
          "unknown fourcc '${StreamHeaderPacket.Companion.fourccToString(fourcc)}' type: $type",
        )
      }
    }
