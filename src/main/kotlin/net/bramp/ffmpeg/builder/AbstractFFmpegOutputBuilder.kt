...
abstract class AbstractFFmpegOutputBuilder<T : AbstractFFmpegOutputBuilder<T>> :
  ...
  fun setAudioBitDepth(bit_depth: String): T {
    return setAudioSampleFormat(bit_depth)
  }

  fun setAudioSampleFormat(sample_format: String): T {
    this.audio_enabled = true
    this.audio_sample_format = net.bramp.ffmpeg.Preconditions.checkNotEmpty(sample_format, "sample format must not be empty")
    return getThis()
  }
  ...
  override fun build(parent: FFmpegBuilder, pass: Int): List<String> {
    ...
    if (targetSize > 0L) {
      ...
      val durationInSeconds = inputProbeResult.format.duration
  ...
  // Public getters for the deprecated fields, if needed for Java interop or specific patterns
  fun getConstantRateFactor(): Double? = this.constantRateFactor
  fun getAudioSampleFormat(): String? = this.audio_sample_format
  fun getAudioBitRate(): Long = this.audio_bit_rate
  fun getAudioQuality(): Double? = this.audio_quality
  fun getAudioBitStreamFilter(): String? = this.audio_bit_stream_filter
  fun getAudioFilter(): String? = this.audio_filter
  fun getVideoBitRate(): Long = this.video_bit_rate
  fun getVideoQuality(): Double? = this.video_quality
  fun getVideoPreset(): String? = this.video_preset
  fun getVideoFilter(): String? = this.video_filter
  fun getVideoBitStreamFilter(): String? = this.video_bit_stream_filter
  fun getComplexFilter(): String? = this.complexFilter
  ...
  companion object {
    ...
    @JvmStatic
    internal fun formatDecimalInteger(d: Double): String {
      return trailingZero.matcher(d.toString()).replaceAll("")
    }
  }
}
